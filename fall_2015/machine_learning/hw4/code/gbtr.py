...
class HousingLearner:
    ...
    def train(self, depth):
        for i in range(10):
            print "Boosting round:",i+1
            rt = RegressionTree(self.X_train, self.Y_train)
            self.T = rt.build(depth)
            if i == 0:
                self.T0 = self.T
            for i in range(len(self.Y_train)):
                self.Y_train[i] -= self.eval_data(self.X_train[i], self.T)
    ...
    def test(self):
        ...
        # Testing error (with boosting)
        total = len(self.X_test)
        mse = 0.0
        for i in range(total):
            mse += ((self.eval_data(self.X_test[i], self.T0) - self.Y_test[i])**2.0)
        mse /= float(total)
        print "Testing MSE:",mse
