...
class Booster:
    ...
    def boost(self, X, Y, T, Yt, threshold, random=False):
        ...
        round_top15 = self.margin(classifier, X, Y)
        gt = [52, 51, 56, 15, 6, 22, 23, 4, 26, 24, 7, 54, 5, 19, 18]
        print "Matching Cheng in Round:", len([x for x in round_top15 if x in gt])
        return round_top15
...
if __name__=="__main__":
    ...
    if args.r is not None:
        top15 = booster.train(random=bool(int(args.r)))
        gt = [52, 51, 56, 15, 6, 22, 23, 4, 26, 24, 7, 54, 5, 19, 18]
        print "Matching Cheng in Total:", len([x for x in top15 if x in gt])
        print top15
