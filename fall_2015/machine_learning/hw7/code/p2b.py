...
from nn import kde, cosine_distance, gaussian_distance, polyd2_distance
...
if args.d == "s":
    ...
    for i in range(1):
        ...
        # Run knn
        H = kde(X, Xi, Y.ravel(), d=gaussian_distance)
        c = np.sum(Yi.ravel()==H)
        print "Accuracy:", float(c)/float(len(Yi))
elif args.d == "d":
    ...
    # Run knn
    for d in [gaussian_distance, polyd2_distance]:
        H = kde(train_data, test_data, train_labels, d=d)
        c = np.sum(test_labels.ravel()==H)
        print "Accuracy:", float(c)/float(len(test_labels))
else:
    pass
